...
server, while keeping other parameters constant. Each load level was
measured for 90 seconds following a warm-up phase of 10 seconds.

The following metrics were recorded:
\begin{itemize}
  \item Achieved total request throughput (requests/second)
  \item Success rate and number of timeouts
  \item End-to-end request 95th percentile latency
\end{itemize}

The results reveal three distinct operating regions, as illustrated in
Figures~\ref{fig:p95-latency}--\ref{fig:success-rate}. At low to moderate
load levels (up to approximately 300~requests/s), the system exhibits
near-linear scaling: achieved throughput closely follows the target rate
(Figure~\ref{fig:throughput}), the request success rate remains at
100\% (Figure~\ref{fig:success-rate}), and the p95 end-to-end latency
stays low and stable, below 200~ms (Figure~\ref{fig:p95-latency}). As the
target load approaches 400~requests/s, the backend reaches its
saturation point. This transition is marked by a sharp increase in tail
latency by more than an order of magnitude, while throughput plateaus at around
420~requests/s and the success rate begins
to decline due to emerging timeouts. Beyond this point, further increases in
offered load push the system into sustained overload: effective
throughput collapses despite higher target rates, p95 latency remains extremely high due to
queue buildup, and the success rate
degrades severely.
...
\end{axis}
\end{tikzpicture}
\caption{Cloud backend p95 latency as a function of \emph{target} request rate.
Latency remains low and stable up to approximately 300~requests/s.
Beyond 400~requests/s, tail latency increases sharply, indicating
saturation and queue buildup. At higher target rates, the system
enters sustained overload with high tail latency.}
...
\end{axis}
\end{tikzpicture}
\caption{Achieved backend throughput versus target request rate.
Throughput scales nearly linearly with load up to approximately
400~requests/s. Beyond this point, the system saturates and collapses,
with achieved throughput decreasing sharply despite increasing
offered load.}
...
400~requests/s. Beyond this point, success rate degrades sharply as
timeouts and failed requests increase under overload conditions.}
...
Figure~\ref{fig:wrk2-root} shows the achieved request rate as a function 
...
\begin{axis}[
  ...
  ylabel={Achieved request rate per user [req/s]},
  ymin=0,
...
\end{axis}
\end{tikzpicture}
\caption{Nginx server response latency across different percentiles (p10, p50, p95) as a function of offered load.
Latency remains low and stable up to roughly 4000~requests/s, indicating efficient handling of concurrent requests. 
