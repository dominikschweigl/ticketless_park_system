...
The cloud backend was deployed on a single Amazon EC2 t3.micro
instance. This instance type provides one virtual CPU and 1~GB of RAM
and relies on a burst-based CPU credit model. No load balancing or
horizontal scaling was used in order to observe the inherent limits of a
single-instance Akka-based backend.

The backend exposes a REST-based HTTP API implemented using Akka HTTP
and Akka Typed actors. All parking-lot state management, payment
processing and booking coordination are handled within the actor system.
...
neither the Akka cloud backend nor any edge servers were involved. In this 
setup, the measured latency and throughput mainly reflect the capability of 
the Nginx server to deliver static frontend assets under concurrent access.

The web server under test was hosted at \texttt{http://54.234.138.208}. The 
test consisted of running a series of load tests at increasing levels of 
concurrent users, from 100 up to 800. Each test lasted 60 seconds, and the 
client utilized six CPU threads to issue requests. The objective was to 
observe how both latency and throughput behaved as the number of simultaneous 
requests increased, and to identify the point at which the server's 
performance began to degrade due to resource limitations.

The results of the load tests show a clear trend in the server’s behavior 
under increasing concurrency. At lower concurrency levels, from 100 to 400 
users, the server handled requests efficiently, with average latency 
remaining under half a second and requests per second increasing steadily 
as more users were added. Beyond 400 concurrent users, the server began to 
experience connection errors, resulting in higher average latency and 
reduced throughput. At concurrency levels of 500 and above, 
many requests failed due to connection attempt timeouts, indicating that 
...
\addlegendentry{10th percentile}
...
\addlegendentry{50th percentile}
...
\begin{tikzpicture}
% Left axis: 200 OK responses
\begin{axis}[
    width=8cm,
    height=8cm,
    xlabel={Concurrent Connections},
    ylabel={Successful 200 OK Responses},
    ybar,
    bar width=11pt,
    xtick=data,
    symbolic x coords={100,200,300,400,500,600,700,800},
    grid=major,
    ymin=0, ymax=55000,
    legend style={at={(0.2,-0.18)},anchor=north,legend columns=2},
]

\addplot+[ybar,fill=blue,draw=none,bar shift=-4pt] coordinates {
    (100,17336)
    (200,29556)
    (300,39056)
    (400,49241)
...
\addlegendentry{200 OK}

\end{axis}

% Right axis: Connection Timeouts
\begin{axis}[
    width=8cm,
    height=8cm,
    xlabel={},
    ylabel={Connection Timeouts},
    ylabel near ticks,
    ybar,
    bar width=11pt,
    xtick=data,
    ...
    axis x line=none,
    ymin=0, ymax=900,
    legend style={at={(0.65,-0.18)},anchor=north,legend columns=2},
]

\addplot+[ybar,fill=red,draw=none,bar shift=4pt] coordinates {
    (100,0)
    (200,0)
    (300,0)
    (400,1)
...
\addlegendentry{Connection Timeout}
